...
} ngx_http_contentcode_ctx_t;

static void *ngx_http_contentcode_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_contentcode_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);

static ngx_command_t  ngx_http_contentcode_filter_commands[] = {
        {
                ngx_string("contentcode"),
                NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_FLAG,
                ngx_conf_set_flag_slot, 
                NGX_HTTP_LOC_CONF_OFFSET,
                offsetof(ngx_http_contentcode_conf_t, enable),
                NULL
        },
        ngx_null_command
    };
...
static ngx_int_t ngx_http_contentcode_header_filter(ngx_http_request_t *r);
static ngx_int_t ngx_http_contentcode_header_filter(ngx_http_request_t *r){

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        "contentcode   header filter start conf enable recheck");


    ngx_http_contentcode_ctx_t       *ctx;
    off_t                       len;
    ngx_http_contentcode_conf_t *conf;
    ...
    conf = ngx_http_get_module_loc_conf(r, ngx_http_contentcode_filter_module);

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        "contentcode header filter  before conf enable recheck");

    if (!conf->enable) return ngx_http_next_header_filter(r);

    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        "contentcode header filter  after conf enable recheck");
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        "contentcode header filter: Allocating %d bytes for data buffer", len);
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
        "contentcode  header filter before ngx_pcalloc");

    len = r->headers_out.content_length_n;
    if (len == -1) {
        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
            "contentcode  header filter Missing Content-Length header causes allocating default buffer size");
    ...
    ngx_http_set_ctx(r, ctx, ngx_http_contentcode_filter_module);
    return NGX_OK;
